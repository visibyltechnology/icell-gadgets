...
export default function AdminLayout() {
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-gray-50 font-sans overflow-x-hidden">
      ...
      <div className="md:hidden bg-[#1A2856] text-white p-4 flex justify-between items-center z-20 shadow-md border-b border-[#E31E24]/20 shrink-0">
        <div>
          <h2 className="font-black text-lg uppercase tracking-widest text-white">
            ICELL<em className="text-[#E31E24] not-italic"> GADGETS</em> <span className="text-gray-400">ADMIN</span>
          </h2>
        </div>
      ...
      {/* Sidebar */}
      <aside className={`
        fixed md:sticky top-0 left-0 h-[100dvh] md:h-screen z-30 w-64 bg-[#1A2856] border-r border-[#1A2856] shadow-xl md:shadow-none
      ...
      `}>
        <div className="p-6 hidden md:block border-b border-[#2A3F8F]">
          <h2 className="font-black text-2xl uppercase tracking-widest text-white mb-1">
            ICELL<em className="text-[#E31E24] not-italic"> GADGETS</em> <span className="text-gray-400">ADMIN</span>
          </h2>
          <p className="text-xs font-medium text-gray-300 break-all">{user.email}</p>
        </div>

        <div className="md:hidden p-6 border-b border-[#2A3F8F] mt-16">
           <p className="text-xs font-medium text-gray-300 break-all">{user.email}</p>
        </div>
            ...
            <NavLink
              ...
              className={({ isActive }) => `
                ...
                ${isActive 
                  ...
                  : 'text-gray-300 hover:bg-[#2A3F8F] hover:text-white'}
              `}
            >
        ...
        <div className="p-4 border-t border-[#2A3F8F]">
          <button
            onClick={async () => {
              await logout();
              toast.success('Signed out successfully');
            }}
            className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-sm uppercase tracking-wider font-bold bg-transparent border border-[#2A3F8F] text-gray-300 hover:bg-[#2A3F8F] hover:text-white transition-all"
          >
